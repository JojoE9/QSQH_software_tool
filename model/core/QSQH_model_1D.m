...
dir_vel_tilde = [workdir,file_separator,'universal_velocity_field_1D',...
    folderending,file_separator];

% The folder that will store the 1D output of the synthetic model
dir_output = [workdir,file_separator,'output_1D',...
    folderending,file_separator];
...
while pw < pw_x || pw < pw_z || pw < pw_t
    ...
    for n = 1:size(tstep,2)
        ...
        idx = randi(size(u_tilde,1),numel(u_tauL),1);

        % [u_tilde_sample,idx] = datasample(u_tilde,numel(u_tauL));
        u_tilde_sample = u_tilde(idx,:);
